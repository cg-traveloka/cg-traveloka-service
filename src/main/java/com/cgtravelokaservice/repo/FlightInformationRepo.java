...
public interface FlightInformationRepo extends JpaRepository<FlightInformation, Integer> {
    ...
    @Query("SELECT fi " +
            ...
            "INNER JOIN fi.seatInformations si " +
            "WHERE (:airPlantBrandId IS NULL OR fi.airPlantBrand.id = :airPlantBrandId) " +
            "AND fi.fromAirPortLocation.id = :fromAirportLocationId " +
            "AND fi.toAirPortLocation.id = :toAirportLocationId " +
            "AND  fi.startTime >=  :startTime " +
            "AND si.quantity >= :seatQuantity " +
            "AND si.seatType.id = :seatTypeId " +
            "AND si.unitPrice >= :priceFrom " +
            "AND si.unitPrice <= COALESCE(:priceTo, 10000000000) " +
            "ORDER BY " +
            "CASE WHEN :sortBy = 'unitPrice' THEN si.unitPrice END, " +
            "CASE WHEN :sortBy = 'startTime' AND :order = 'desc' THEN fi.startTime END DESC, " +
            "CASE WHEN :sortBy = 'endTime' AND :order = 'desc' THEN fi.endTime END DESC, " +
            "CASE WHEN :sortBy = 'startTime' THEN fi.startTime END, " +
            "CASE WHEN :sortBy = 'endTime' THEN fi.endTime END ")
    ...
    default List<FlightInformation> searchForList(Integer fromAirportLocationId, Integer toAirportLocationId,
                                                  LocalDateTime startTime, Integer airPlantBrandId,
