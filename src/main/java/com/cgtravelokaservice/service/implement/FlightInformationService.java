...
public class FlightInformationService implements IFlightInformationService {
    ...
    private final AirplaneBrandService
    ...
    public FlightInformationService(FlightInformationRepo flightInformationRepo, AirplaneBrandService airplaneBrandService, SeatService seatService, IConvertUtil convertUtil, AirplaneBrandRepo airplaneBrandRepo) {
    ...
    public List <FlightInfoSearchDTO> searchFlights(SearchFlightDetailsRequestDTO request, Pageable pageable) {
        ...
        Slice <FlightInformation>
                flightInformationSlice =
                flightInformationRepo.search(request.getFromAirportLocationId(), request.getToAirportLocationId(), request.getStartDate().atStartOfDay(), request.getAirPlantBrandId(), request.getSeatTypeId(), request.getSeatQuantity(), request.getSortBy(), request.getOrder(), request.getPriceFrom(), request.getPriceTo(), pageable);
        List <FlightInformation>
                flightInformation =
                new ArrayList <>(flightInformationSlice.getContent());
        return flightInformation.stream().map(flightInfo -> convertUtil.convertToFlightDetailsDTO(flightInfo, request.getSeatTypeId())).toList();
    }


    @Override
    public List <FlightInformation> searchList(SearchFlightDetailsRequestDTO request) {
        return flightInformationRepo.searchForList(request.getFromAirportLocationId(), request.getToAirportLocationId(), request.getStartDate().atStartOfDay(), request.getAirPlantBrandId(), request.getSeatTypeId(), request.getSeatQuantity());
    ...
    public SearchFlightResponse loadSearchFlightResponse(SearchFlightDetailsRequestDTO request) {
        ...
        List <AirPlantBrand> airPlantBrands =
                airplaneBrandService.findByFlightInfos(flightInformationList);

        List <AirPlantSearchDTO>
                airPlantSearchDTO =
                airPlantBrands.stream().map(brand -> new AirPlantSearchDTO(brand.getId(), brand.getName(), brand.getLogoUrl())).collect(Collectors.toList());

        String value1 =
                seatService.getLowestPriceSeat(flightInformationList, request.getSeatTypeId()).getUnitPrice().toString();
        String value2 =
                seatService.getShortestFlight(flightInformationList, request.getSeatTypeId()).getUnitPrice().toString();
        FlightInForShortDescription description =
                FlightInForShortDescription.builder().name("Gía thấp nhất").unitPrice(value1).build();
        FlightInForShortDescription description2 =
                FlightInForShortDescription.builder().name("Thời gian bay ngắn " + "nhất").unitPrice(value2).build();
        List <FlightInForShortDescription>
    ...
    private List <FlightInfoSearchDTO> searchFlightsSortByDuration(SearchFlightDetailsRequestDTO request, Pageable pageable) {
        List <FlightInformation> flights =
                searchList(request);
        List <FlightInformation> sortedFlights =
                new ArrayList <>(flights);
        sortedFlights.sort(Comparator.comparingLong(flight -> ChronoUnit.MINUTES.between(flight.getStartTime(), flight.getEndTime())));
        int max =
                Math.min((pageable.getPageNumber() + 1) * pageable.getPageSize(), flights.size());
        List <FlightInformation> result =
                sortedFlights.subList(pageable.getPageNumber(), max);
        return result.stream().map(flight -> convertUtil.convertToFlightDetailsDTO(flight, request.getSeatTypeId())).toList();
